...
class TriangoliRettangoli(Scene):
        ...
        for epoch in [1, 10, 75]:
            ...
            for triangolo_input in tutti_triangoli:
                ...
                if len(triangolo_input) >= 5:
                    for etichetta in triangolo_input[2:]:  # Salta triangolo e angolo_retto
            ...
            for triangolo_input, triangolo_output in zip(tutti_triangoli, tutti_triangoli_output):
                # Ottieni la posizione del poligono triangolo (primo elemento) in ogni gruppo
                poligono_triangolo_input = triangolo_input[0]  # Triangolo è primo elemento
                poligono_triangolo_output = triangolo_output[0]  # Triangolo è primo elemento
                
                # Calcola dove si trova il vertice dell'angolo retto per ogni triangolo
                vertice_retto_input = poligono_triangolo_input.get_vertices()[0]  # Punto A (angolo retto)
                vertice_retto_output = poligono_triangolo_output.get_vertices()[0]  # Punto A (angolo retto)
                
                # Calcola l'offset necessario per allineare i vertici degli angoli retti
                offset = vertice_retto_input - vertice_retto_output
                animazioni_sovrapposizione.append(triangolo_output.animate.shift(offset))
            
            self.play(*animazioni_sovrapposizione, run_time=2)
            self.wait(1)
            
            # Mostra analisi errore invece dell'ipotenusa predetta
            testo_errore_medio = self.mostra_analisi_errore(tutti_triangoli_output, dati_triangoli, ipotenuse_predette)
            self.wait(3)
    ...
    def ottieni_predizioni_nn(self, dati_triangoli, epoch=1):
        ...
        try:
            ...
            print(f"Avvertimento: Modello epoca 1 non trovato ({e}), uso approssimazione")
    ...
    def mostra_analisi_errore(self, triangoli_output, dati_triangoli, ipotenuse_predette):
        ...
        errore_medio_pct = sum(percentuali_errore) / len(percentuali_errore) if percentuali_errore else 0
        
        # Sostituisci etichette ipotenusa con etichette errore
        animazioni_trasformazione = []
        
        for i, (triangolo_output, errore, errore_pct) in enumerate(zip(triangoli_output, errori, percentuali_errore)):
            # Trova etichetta ipotenusa (ultimo elemento nel gruppo)
            etichetta_ipo = triangolo_output[-1]  # Ultimo elemento dovrebbe essere etichetta_c
            ...
            animazioni_trasformazione.append(Transform(etichetta_ipo, nuova_etichetta_errore))
        
        # Mostra trasformazioni
        self.play(*animazioni_trasformazione, run_time=1.5)
        
        # Aggiungi visualizzazione errore medio
        testo_errore_medio = Text(f"Errore Medio: {errore_medio_pct:.1f}%", font_size=32, color=RED)
